package org.kcoin.walletserver;
...
import org.kcoin.api.GrpcAPI.TransactionSignWeight.Result.response_code;
...
import org.kcoin.common.utils.TransactionUtils;
...
import org.kcoin.protos.contract.StorageContract.BuyStorageBytesContract;
...
public class WalletApi {
  ...
  private Transaction signTransaction(Transaction transaction)
    ...
    while (true) {
      ...
      byte[] passwd = org.kcoin.keystore.StringUtils.char2Byte(password);
      org.kcoin.keystore.StringUtils.clear(password);
      if (isEckey) {
      ...
      org.kcoin.keystore.StringUtils.clear(passwd);
  ...
  public byte[] generateContractAddress(byte[] ownerAddress, Transaction sym) {
    // get tx hash
    byte[] txRawDataHash = Sha256Sm3Hash.of(sym.getRawData().toByteArray()).getBytes();
  ...
  public static boolean sendShieldedCoinWithoutAsk(PrivateParametersWithoutAsk privateParameters,
    ...
    BytesMessage symHash = rpcCli.getShieldedTransactionHash(transactionExtention.getTransaction());
    ...
    for (int i = 0; i < spendDescList.size(); i++) {
      ...
      builder.setTxHash(ByteString.copyFrom(symHash.getValue().toByteArray()));
